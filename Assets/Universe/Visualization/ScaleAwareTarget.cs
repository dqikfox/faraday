...
namespace RealityEngine.Visualization
{
    ...
    public sealed class ScaleAwareTarget : MonoBehaviour
    {
        ...
        public void ApplyScale(int scale)
        {
            ...
            if (_lensTarget != null)
                _lensTarget.SetScale(ScaleEngine.ClampMicro(_scale));
            UpdateHonesty();
        }
        ...
        void UpdateHonesty()
        {
            if (_honesty == null)
                return;
            string extra = "";
            if (ScaleEngine.IsCosmos(_scale))
            {
                ScaleLevel cosmos = (ScaleLevel)ScaleEngine.ClampScale(_scale);
                if (cosmos == ScaleLevel.Room)
                    extra = "\nRoom schematic — lab table + local objects";
                else if (cosmos == ScaleLevel.Planetary)
                    extra = "\nGeographic toy Earth — Giza marker schematic";
                else if (cosmos == ScaleLevel.Solar)
                    extra = "\nKepler toy — Sun + Earth orbit schematic";
            }
            else
            {
                ScaleLevel L = (ScaleLevel)ScaleEngine.ClampMicro(_scale);
