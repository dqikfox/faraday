...
namespace RealityEngine.Visualization
{
    ...
    /// Minkhaf (G 7430-7440) East Field elite double mastaba south of Ankhhaf (Lehner / Reisner),
    ...
    public static class GizaField
    {
        ...
        public const string MinkhafName = "Minkhaf";
        ...
        // Babaef / Khnumbaf (G 5230): central West Field elite mastaba (Lehner / Reisner).
        // South of Hemiunu strip; east offering chapel schematic. Vizier, late 4th Dynasty.
        public const float BabaefEastInsetFromWestEastEdgeM = 48f;
        public const float BabaefNorthFrac = 0.45f;
        public const float BabaefBodyEW = 32.0f;
        public const float BabaefBodyNS = 18.0f;
        public const float BabaefBodyHM = 8.5f;
        public const float BabaefUpperEW = 29.0f;
        public const float BabaefUpperNS = 16.0f;
        public const float BabaefUpperHM = 1.05f;
        public const float BabaefChapelEW = 9.5f;
        public const float BabaefChapelNS = 6.8f;
        public const float BabaefChapelHM = 4.3f;

        // Duaenre (G 5110): southern West Field elite mastaba (son of Khafre; Lehner / Reisner).
        // South of Babaef strip; east offering chapel schematic.
        public const float DuaenreEastInsetFromWestEastEdgeM = 52f;
        public const float DuaenreNorthFrac = 0.26f;
        public const float DuaenreBodyEW = 34.0f;
        public const float DuaenreBodyNS = 19.5f;
        public const float DuaenreBodyHM = 8.8f;
        public const float DuaenreUpperEW = 31.0f;
        public const float DuaenreUpperNS = 17.5f;
        public const float DuaenreUpperHM = 1.05f;
        ...
        // Minkhaf (G 7430-7440): East Field elite double mastaba south of Ankhhaf / SE strip (Lehner / Reisner).
        // Son of Khufu; N-S twin body G7430/G7440; east offering chapel schematic.
        public const float MinkhafWestInsetFromEastWestEdgeM = 46f;
        public const float MinkhafNorthFrac = 0.30f;
        public const float MinkhafBodyEW = 19.0f;
        public const float MinkhafBodyNS = 38.0f;
        public const float MinkhafBodyHM = 7.5f;
        public const float MinkhafUpperEW = 16.5f;
        public const float MinkhafUpperNS = 35.0f;
        public const float MinkhafUpperHM = 1.0f;
        public const float MinkhafChapelEW = 9.0f;
        public const float MinkhafChapelNS = 6.5f;
        public const float MinkhafChapelHM = 4.2f;
        ...
        public static void ExpandExtents(ref float xMin, ref float xMax, ref float zMin, ref float zMax)
        {
            ...
            LayoutBabaef(out float babEast, out float babNorth);
            Enc(ref xMin, ref xMax, ref zMin, ref zMax, babEast, babNorth,
                BabaefBodyEW * 0.5f + BabaefChapelEW + 14f, BabaefBodyNS * 0.5f + 12f);

            LayoutDuaenre(out float duaEast, out float duaNorth);
            ...
            LayoutMinkhaf(out float minkEast, out float minkNorth);
            Enc(ref xMin, ref xMax, ref zMin, ref zMax, minkEast, minkNorth,
                MinkhafBodyEW * 0.5f + MinkhafChapelEW + 16f, MinkhafBodyNS * 0.5f + 12f);
        ...
        public static void ForceRebuildAll()
        {
            ...
            DestroyNamed(GizaComplex.FindNamed(MinkhafName));
        ...
        public static void EnsureBabaef(GizaComplex.Pose pose)
        {
            GameObject old = GizaComplex.FindNamed(BabaefName);
            if (old != null && old.transform.Find(BabaefName + MassingMarker) == null)
                DestroyNamed(old);
            Ensure(BabaefName, pose, BuildBabaef, pose.surfaceY, true);
        }

        public static void EnsureDuaenre(GizaComplex.Pose pose)
        {
            GameObject old = GizaComplex.FindNamed(DuaenreName);
        ...
        public static void EnsureMinkhaf(GizaComplex.Pose pose)
        {
            GameObject old = GizaComplex.FindNamed(MinkhafName);
            if (old != null && old.transform.Find(MinkhafName + MassingMarker) == null)
                DestroyNamed(old);
            Ensure(MinkhafName, pose, BuildMinkhaf, pose.surfaceY, true);
        }
        ...
        static void LayoutBabaef(out float east, out float north)
        {
            LayoutWest(out float east0, out float east1, out float north0, out float north1);
            // G 5230 central West Field: south of Hemiunu, slightly deeper west (Lehner / Reisner).
            east = east1 - BabaefEastInsetFromWestEastEdgeM;
            north = north0 + (north1 - north0) * BabaefNorthFrac;
        }

        static void LayoutDuaenre(out float east, out float north)
        {
            LayoutWest(out float east0, out float east1, out float north0, out float north1);
            // G 5110 southern West Field: south of Babaef strip (Lehner / Reisner).
        ...
        static void LayoutMinkhaf(out float east, out float north)
        {
            LayoutEast(out float east0, out float east1, out float north0, out float north1);
            // G 7430-7440 south of Ankhhaf in Eastern Cemetery SE strip (Lehner / Reisner).
            east = east0 + MinkhafWestInsetFromEastWestEdgeM + MinkhafBodyEW * 0.5f;
            north = north0 + (north1 - north0) * MinkhafNorthFrac;
        }
        ...
        static GameObject BuildBabaef(GizaComplex.Pose pose)
        {
            LayoutBabaef(out float east, out float north);
            Vector3 world = GizaComplex.WorldFromKhufu(pose, east, north, 0f);
            GameObject root = GizaBuild.Root(BabaefName, pose.parent, world, pose.rot);
            Material lime = GizaBuild.InteriorLime();
            Material mud = GizaBuild.Mudbrick();
            Material sand = GizaBuild.DesertSand();
            Material pav = GizaBuild.Pavement();

            float bodyEW = BabaefBodyEW;
            float bodyNS = BabaefBodyNS;
            float bodyH = BabaefBodyHM;
            float halfE = bodyEW * 0.5f;
            float chapelEW = BabaefChapelEW;
            float chapelNS = BabaefChapelNS;
            float chapelH = BabaefChapelHM;

            var apron = new LabMeshBuilder(8, 12);
            apron.AddBox(new Vector3(chapelEW * 0.35f, 0.06f, 0f),
                new Vector3(bodyEW + chapelEW + 16f, 0.12f, bodyNS + 14f), Color.white);
            GizaBuild.SpawnMesh(root.transform, BabaefName + "_Apron",
                apron.Build(BabaefName + "_Apron"), sand, true);

            var body = new LabMeshBuilder(8, 12);
            body.AddBox(new Vector3(0f, bodyH * 0.5f, 0f), new Vector3(bodyEW, bodyH, bodyNS), Color.white);
            GizaBuild.SpawnMesh(root.transform, BabaefName + "_Body",
                body.Build(BabaefName + "_Body"), lime, true);

            var cornice = new LabMeshBuilder(8, 12);
            cornice.AddBox(new Vector3(0f, bodyH + BabaefUpperHM * 0.5f, 0f),
                new Vector3(BabaefUpperEW, BabaefUpperHM, BabaefUpperNS), Color.white);
            GizaBuild.SpawnMesh(root.transform, BabaefName + "_Cornice",
                cornice.Build(BabaefName + "_Cornice"), mud, true);
            ...
            chapelShell.AddBox(new Vector3(chapelX, deckY + chapelH + 0.16f, 0f),
                new Vector3(chapelEW + 0.3f, 0.32f, chapelNS + 0.3f), Color.white);
            GizaBuild.SpawnMesh(root.transform, BabaefName + "_Chapel",
                chapelShell.Build(BabaefName + "_Chapel"), lime, true);
            ...
            GizaBuild.SpawnMesh(root.transform, BabaefName + "_ChapelInterior",
                interior.Build(BabaefName + "_ChapelInterior"), lime, true);
            ...
            corridor.AddBox(new Vector3(halfE + corrLen * 0.5f, deckY + 0.1f, 0f),
                new Vector3(corrLen, 0.2f, doorW + 1.4f), Color.white);
            GizaBuild.SpawnMesh(root.transform, BabaefName + "_Corridor",
                corridor.Build(BabaefName + "_Corridor"), pav, true);

            var mark = new LabMeshBuilder(8, 12);
            mark.AddBox(new Vector3(0f, 0.12f, 0f), new Vector3(0.5f, 0.24f, 0.5f), Color.white);
            GizaBuild.SpawnMesh(root.transform, BabaefName + MassingMarker,
                mark.Build(BabaefName + MassingMarker), pav, false);

            const string honesty =
                GizaComplex.HonestyPrefix + "\n" +
                "Babaef / Khnumbaf (G 5230). Late 4th Dynasty vizier - Western Cemetery elite mastaba.\n" +
                "Lehner / Reisner schematic massing (~32 x 18 x 8.5 m limestone body + cornice, east offering chapel).\n" +
                "Not photogrammetry. Not proven interior chambers beyond the schematic chapel.";
            GizaBuild.HonestyPlate(root.transform, BabaefName + "_Honesty", honesty, 26f);
            Transform plate = root.transform.Find(BabaefName + "_Honesty");
            if (plate != null)
            {
                plate.localPosition = new Vector3(halfE + chapelEW + 3.5f, 1.6f, chapelNS * 0.5f + 2.5f);
                plate.localRotation = Quaternion.Euler(0f, 90f, 0f);
            ...
            const string chapelText =
                ...
                "bAbA.f / Xnmw-bA.f (Babaef / Khnumbaf). Vizier; G 5230 Western Cemetery.\n" +
                "No invented hieroglyph glyphs (TMP lacks Egyptian font).\n" +
                "Source: published Giza mastaba corpus - not AI-invented text.";
            GizaBuild.HonestyPlate(root.transform, BabaefName + "_ChapelText", chapelText, 22f);
            Transform textPlate = root.transform.Find(BabaefName + "_ChapelText");
            if (textPlate != null)
            {
        ...
        static GameObject BuildDuaenre(GizaComplex.Pose pose)
        {
            LayoutDuaenre(out float east, out float north);
            Vector3 world = GizaComplex.WorldFromKhufu(pose, east, north, 0f);
            GameObject root = GizaBuild.Root(DuaenreName, pose.parent, world, pose.rot);
            Material lime = GizaBuild.InteriorLime();
            Material mud = GizaBuild.Mudbrick();
            Material sand = GizaBuild.DesertSand();
            Material pav = GizaBuild.Pavement();

            float bodyEW = DuaenreBodyEW;
            float bodyNS = DuaenreBodyNS;
            float bodyH = DuaenreBodyHM;
            float halfE = bodyEW * 0.5f;
            float chapelEW = DuaenreChapelEW;
            float chapelNS = DuaenreChapelNS;
            float chapelH = DuaenreChapelHM;

            var apron = new LabMeshBuilder(8, 12);
            apron.AddBox(new Vector3(chapelEW * 0.35f, 0.06f, 0f),
                new Vector3(bodyEW + chapelEW + 16f, 0.12f, bodyNS + 14f), Color.white);
            GizaBuild.SpawnMesh(root.transform, DuaenreName + "_Apron",
                apron.Build(DuaenreName + "_Apron"), sand, true);

            var body = new LabMeshBuilder(8, 12);
            body.AddBox(new Vector3(0f, bodyH * 0.5f, 0f), new Vector3(bodyEW, bodyH, bodyNS), Color.white);
            GizaBuild.SpawnMesh(root.transform, DuaenreName + "_Body",
                body.Build(DuaenreName + "_Body"), lime, true);

            var cornice = new LabMeshBuilder(8, 12);
            cornice.AddBox(new Vector3(0f, bodyH + DuaenreUpperHM * 0.5f, 0f),
                new Vector3(DuaenreUpperEW, DuaenreUpperHM, DuaenreUpperNS), Color.white);
            GizaBuild.SpawnMesh(root.transform, DuaenreName + "_Cornice",
                cornice.Build(DuaenreName + "_Cornice"), mud, true);
            ...
            chapelShell.AddBox(new Vector3(chapelX, deckY + chapelH + 0.16f, 0f),
                new Vector3(chapelEW + 0.3f, 0.32f, chapelNS + 0.3f), Color.white);
            GizaBuild.SpawnMesh(root.transform, DuaenreName + "_Chapel",
                chapelShell.Build(DuaenreName + "_Chapel"), lime, true);

            var interior = new LabMeshBuilder(40, 60);
            interior.AddRoom(new Vector3(chapelX, deckY + floorT + 1.7f, 0f),
                new Vector3(chapelEW - wallT * 2f - 0.35f, 3.4f, chapelNS - wallT * 2f - 0.5f),
                Color.white, false, false, true, true);
            GizaBuild.SpawnMesh(root.transform, DuaenreName + "_ChapelInterior",
                interior.Build(DuaenreName + "_ChapelInterior"), lime, true);
            ...
            corridor.AddBox(new Vector3(halfE + corrLen * 0.5f, deckY + 0.1f, 0f),
                new Vector3(corrLen, 0.2f, doorW + 1.4f), Color.white);
            GizaBuild.SpawnMesh(root.transform, DuaenreName + "_Corridor",
                corridor.Build(DuaenreName + "_Corridor"), pav, true);

            var mark = new LabMeshBuilder(8, 12);
            mark.AddBox(new Vector3(0f, 0.12f, 0f), new Vector3(0.5f, 0.24f, 0.5f), Color.white);
            GizaBuild.SpawnMesh(root.transform, DuaenreName + MassingMarker,
                mark.Build(DuaenreName + MassingMarker), pav, false);

            const string honesty =
                GizaComplex.HonestyPrefix + "\n" +
                "Duaenre (G 5110). Son of Khafre - Western Cemetery elite mastaba.\n" +
                "Lehner / Reisner schematic massing (~34 x 19.5 x 8.8 m limestone body + cornice, east offering chapel).\n" +
                "Not photogrammetry. Not proven interior chambers beyond the schematic chapel.";
            GizaBuild.HonestyPlate(root.transform, DuaenreName + "_Honesty", honesty, 26f);
            Transform plate = root.transform.Find(DuaenreName + "_Honesty");
            if (plate != null)
            {
                plate.localPosition = new Vector3(halfE + chapelEW + 3.5f, 1.6f, chapelNS * 0.5f + 2.5f);
                plate.localRotation = Quaternion.Euler(0f, 90f, 0f);
            ...
            const string chapelText =
                "Name plate, Latin transliteration only - Reisner / Lehner Giza corpus:\n" +
                "dwA-n-ra (Duaenre). King's son; G 5110 Western Cemetery.\n" +
                "No invented hieroglyph glyphs (TMP lacks Egyptian font).\n" +
                "Source: published Giza mastaba corpus - not AI-invented text.";
        ...
        static GameObject BuildMinkhaf(GizaComplex.Pose pose)
        {
            LayoutMinkhaf(out float east, out float north);
            Vector3 world = GizaComplex.WorldFromKhufu(pose, east, north, 0f);
            GameObject root = GizaBuild.Root(MinkhafName, pose.parent, world, pose.rot);
            Material lime = GizaBuild.InteriorLime();
            Material mud = GizaBuild.Mudbrick();
            Material sand = GizaBuild.DesertSand();
            Material pav = GizaBuild.Pavement();

            float bodyEW = MinkhafBodyEW;
            float bodyNS = MinkhafBodyNS;
            float bodyH = MinkhafBodyHM;
            float halfE = bodyEW * 0.5f;
            float halfN = bodyNS * 0.5f;
            float chapelEW = MinkhafChapelEW;
            float chapelNS = MinkhafChapelNS;
            float chapelH = MinkhafChapelHM;

            var apron = new LabMeshBuilder(8, 12);
            apron.AddBox(new Vector3(chapelEW * 0.35f, 0.06f, 0f),
                new Vector3(bodyEW + chapelEW + 18f, 0.12f, bodyNS + 16f), Color.white);
            GizaBuild.SpawnMesh(root.transform, MinkhafName + "_Apron",
                apron.Build(MinkhafName + "_Apron"), sand, true);

            // N-S double mastaba: two limestone lobes + mid seam (G 7430 / G 7440 schematic).
            float lobeNS = bodyNS * 0.48f;
            float lobeGap = bodyNS * 0.02f;
            var body = new LabMeshBuilder(24, 36);
            body.AddBox(new Vector3(0f, bodyH * 0.5f, halfN * 0.5f + lobeGap * 0.25f),
            ...
            body.AddBox(new Vector3(0f, bodyH * 0.42f, 0f),
                new Vector3(bodyEW * 0.92f, bodyH * 0.55f, bodyNS * 0.08f), Color.white);
            GizaBuild.SpawnMesh(root.transform, MinkhafName + "_Body",
                body.Build(MinkhafName + "_Body"), lime, true);

            var cornice = new LabMeshBuilder(8, 12);
            cornice.AddBox(new Vector3(0f, bodyH + MinkhafUpperHM * 0.5f, 0f),
                new Vector3(MinkhafUpperEW, MinkhafUpperHM, MinkhafUpperNS), Color.white);
            GizaBuild.SpawnMesh(root.transform, MinkhafName + "_Cornice",
                cornice.Build(MinkhafName + "_Cornice"), mud, true);
            ...
            chapelShell.AddBox(new Vector3(chapelX, deckY + chapelH + 0.16f, 0f),
                new Vector3(chapelEW + 0.3f, 0.32f, chapelNS + 0.3f), Color.white);
            GizaBuild.SpawnMesh(root.transform, MinkhafName + "_Chapel",
                chapelShell.Build(MinkhafName + "_Chapel"), lime, true);

            var interior = new LabMeshBuilder(40, 60);
            interior.AddRoom(new Vector3(chapelX, deckY + floorT + 1.65f, 0f),
                new Vector3(chapelEW - wallT * 2f - 0.35f, 3.3f, chapelNS - wallT * 2f - 0.5f),
                Color.white, false, false, true, true);
            GizaBuild.SpawnMesh(root.transform, MinkhafName + "_ChapelInterior",
                interior.Build(MinkhafName + "_ChapelInterior"), lime, true);
            ...
            corridor.AddBox(new Vector3(halfE + corrLen * 0.5f, deckY + 0.1f, 0f),
                new Vector3(corrLen, 0.2f, doorW + 1.4f), Color.white);
            GizaBuild.SpawnMesh(root.transform, MinkhafName + "_Corridor",
                corridor.Build(MinkhafName + "_Corridor"), pav, true);

            var mark = new LabMeshBuilder(8, 12);
            mark.AddBox(new Vector3(0f, 0.12f, 0f), new Vector3(0.5f, 0.24f, 0.5f), Color.white);
            GizaBuild.SpawnMesh(root.transform, MinkhafName + MassingMarker,
                mark.Build(MinkhafName + MassingMarker), pav, false);

            const string honesty =
                GizaComplex.HonestyPrefix + "\n" +
                "Minkhaf (G 7430-7440). Son of Khufu - Eastern Cemetery elite double mastaba.\n" +
                "Lehner / Reisner schematic massing (~19 x 38 x 7.5 m N-S twin body + cornice, east offering chapel).\n" +
                "Not photogrammetry. Not proven chambers beyond the schematic chapel.";
            GizaBuild.HonestyPlate(root.transform, MinkhafName + "_Honesty", honesty, 28f);
            Transform plate = root.transform.Find(MinkhafName + "_Honesty");
            if (plate != null)
            {
                plate.localPosition = new Vector3(halfE + chapelEW + 3.5f, 1.6f, chapelNS * 0.5f + 2.5f);
                plate.localRotation = Quaternion.Euler(0f, 90f, 0f);
            ...
            const string chapelText =
                "Name plate, Latin transliteration only - Reisner / Lehner Giza corpus:\n" +
                "Mn-xA.f (Minkhaf). Son of Khufu; G 7430-7440 Eastern Cemetery.\n" +
                "No invented hieroglyph glyphs (TMP lacks Egyptian font).\n" +
                "Source: published Giza mastaba corpus - not AI-invented text.";
            GizaBuild.HonestyPlate(root.transform, MinkhafName + "_ChapelText", chapelText, 22f);
            Transform textPlate = root.transform.Find(MinkhafName + "_ChapelText");
            if (textPlate != null)
            {
                textPlate.localPosition = new Vector3(chapelX, 1.45f, -(chapelNS * 0.5f + 2.2f));
                textPlate.localRotation = Quaternion.Euler(0f, 0f, 0f);
