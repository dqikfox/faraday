...
{
    /// <summary>
    /// Places the Reality Engine v0.5 induction lab (with Field Lens + Scale Engine) into the currently open scene.
    ...
    public static class RealityEngineInductionLabMenu
    {
        ...
        public static void PlaceInductionLab()
        {
            ...
            if (existing != null)
            {
                ...
                Debug.Log("Induction Lab already exists. Field Lens + Scale Engine v0.5 ensured on: " + existing.gameObject.name, existing);
                return;
            }
            ...
            Debug.Log(
                "Placed Induction Lab v0.5 (Field Lens + Scale Engine) in scene '" + go.scene.name +
